    ...
    <main className="page-shell dir-rtl !max-w-4xl">
      ...
      <section className="glass-sheen glass-violet space-y-6 p-6 sm:p-8"><div className="flex items-start gap-3 rounded-2xl border border-amber-400/20 bg-amber-400/5 p-4 text-right text-xs leading-7 text-amber-100"><Sparkles className="mt-1 shrink-0 text-amber-300" size={18} /><span>الأدمن يحدد طريقة كل مرحلة. لو السواعد مطلوبون، انتظروا منهم QR التالي. ولو لا، استخدموا الـclue وابحثوا عن QR بأنفسكم.</span></div>
        ...
        {session && <>
          ...
          {completed ? <div className="py-12 text-center"><CheckCircle2 className="mx-auto mb-4 text-emerald-400" size={48} /><h2 className="text-2xl font-black text-white">وصلتم إلى النهاية</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-8 text-slate-400">أبلغوا السواعد أن فريقكم أنهى الرحلة. لا توجد نقاط أو عملات؛ الإنجاز هو إكمال التحديات معًا.</p></div> : stage ? <div className="space-y-5"><div className="rounded-3xl border border-cyan-400/25 bg-cyan-400/10 p-5 text-right"><div className="mb-3 flex flex-wrap items-center justify-between gap-3"><span className="rounded-full border border-cyan-300/30 px-3 py-1 text-[10px] font-black text-cyan-100">المرحلة {stage.index + 1} · {stage.taskType}</span><span className={`rounded-full border px-3 py-1 text-[10px] font-black ${stage.requiresSawaed ? 'border-amber-300/30 text-amber-100' : 'border-emerald-300/30 text-emerald-100'}`}>{stage.requiresSawaed ? 'اعتماد السواعد' : 'clue والبحث الذاتي'}</span></div><h2 className="text-2xl font-black text-white">{stage.title}</h2><p className="mt-4 text-sm leading-8 text-slate-200">{stage.task}</p></div>{stage.clue && <div className="rounded-2xl border border-emerald-400/25 bg-emerald-400/10 p-4 text-right text-sm leading-7 text-emerald-100"><strong className="text-emerald-200">Clue المكان: </strong>{stage.clue}</div>}<div className="rounded-2xl border border-white/10 bg-black/10 p-4 text-right text-sm leading-7 text-slate-300"><strong className="text-amber-200">ماذا بعد؟ </strong>{stage.handoff}</div>{lastStage ? <button type="button" onClick={finish} disabled={busy} className="btn-ember w-full">{busy ? 'جاري الإنهاء...' : 'أنهِ الرحلة بعد إتمام المهمة'}</button> : stage.requiresSawaed ? readyToScan ? <ScannerPanel {...scannerProps} /> : <button type="button" onClick={() => setReadyToScan(true)} disabled={busy} className="btn-violet w-full">استلمنا QR التالي — فتح الكاميرا</button> : <ScannerPanel {...scannerProps} />}</div> : <div className="space-y-4"><div className="rounded-2xl border border-white/10 bg-black/10 p-5 text-center text-sm leading-8 text-slate-300">ابدأوا من QR المرحلة الأولى الذي يسلّمه لكم فريق السواعد.</div>{readyToScan && <ScannerPanel {...scannerProps} />}</div>}
        </>}
      </section>
    </main>
  );
