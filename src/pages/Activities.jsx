...
function CompetitionCard({ competition, onEnter, index }) {
  ...
  return (
    <motion.article initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }} className={`overflow-hidden rounded-[2rem] border backdrop-blur-xl ${toneClasses[meta.tone]}`}>
      <div className="flex items-start justify-between gap-4 p-6"><div className="text-right"><span className="mb-3 inline-flex items-center gap-1.5 rounded-full border border-current/20 px-3 py-1 text-[11px] font-black"><span className={`h-2 w-2 rounded-full ${isReady ? 'animate-pulse bg-emerald-400' : 'bg-slate-500'}`} />{statusText(competition)}</span><h2 className="text-xl font-black text-white">{meta.label}</h2><p className="mt-2 text-sm leading-7 text-slate-300">{meta.description || competition.description}</p></div><Trophy className="shrink-0 text-amber-300/60" size={28} /></div>
