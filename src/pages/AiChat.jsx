...
const AiChat = () => {
  ...
  const submit = async event => {
    ...
    try {
      ...
      const retryHint = err.status === 429 && err.retryAfter ? ` جرّب بعد ${err.retryAfter} ثانية.` : '';
      setError(err.message ? `${err.message}${retryHint}` : 'مساعد الذكاء الاصطناعي غير مفعل بعد');
